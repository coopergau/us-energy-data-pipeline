...
3. (Transform) Within Snowflake, dbt was used to transform the raw dataframes into staging models by typecasting certain columns and testing for null values.
...
Both the heating degree days (HDDs) and cooling degree days (CDDs) pages tell a similar story. From 2010 to 2022 they have both been relatively consistent: The largest decrease in HDDs was about 10% form the average, while the largest decrease in CDDs was about 12% from the average. For the largest increases from the average, HDDs went up about 9% while CDDs went up almost 8%. The split line graph analyzes if these trends were consistent across all regions. Though the absolute amounts are quite different from region to region, it can be seen that the trends are very similar for each respecting page. Looking at the pages separately, it is evident that the amounts of annual HDDs and CDDs have not changed much. Looking a them together though, highlights their difference in absolute values. There have been significantly more HDDs than CDDs over this time frame with the average annual amount of HDDs being almost four times that of CDDs.